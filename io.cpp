...
uint32_t io_ranged_attack(dungeon *d) {
  ...
  if (d->PC->eq[2] == NULL) {
    mvprintw(22,9,"No ranged weapon in inventory.");
    return 1;
  }
  else {
  pair_t dest;
  ...
  mvprintw(0, 0,
           "Choose a monster to attack. 'g' while hovering over the monster to attack, or escape key to cancel.");
  ...
  * not otherwise used.                                                 */
    ...
    if (c == 'g') {
      uint32_t illuminated;
      illuminated = is_illuminated(d->PC,
                                   dest[dim_y],
				   dest[dim_x]);
      if (!illuminated) {
	mvprintw(1,0, "Location not visible");
      }
      if (charpair(dest)) {
	character *c = d->character_map[dest[dim_y]][dest[dim_x]];
	if (d->PC == c) {
	  io_queue_message("You shoot straight up and hit yourself.");
	}
	do_combat(d, d->PC, c);
	fin = 1;
      }
      else {
	mvprintw(1,0,"No monster at location.");
      }
    }
  } while (fin == 0 && c != 27);
